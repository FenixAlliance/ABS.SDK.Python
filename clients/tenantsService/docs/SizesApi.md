...
# **patch_tenant_size**
> EmptyEnvelope patch_tenant_size(tenant_id, tenant_size_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a tenant size
        api_response = api_instance.patch_tenant_size(tenant_id, tenant_size_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of SizesApi->patch_tenant_size:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SizesApi->patch_tenant_size: %s\n" % e)
```
