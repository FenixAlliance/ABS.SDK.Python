...
# **patch_tenant_unit**
> EmptyEnvelope patch_tenant_unit(tenant_id, tenant_unit_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a tenant unit
        api_response = api_instance.patch_tenant_unit(tenant_id, tenant_unit_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of UnitsApi->patch_tenant_unit:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling UnitsApi->patch_tenant_unit: %s\n" % e)
```
