...
> EmptyEnvelope patch_tenant_position(tenant_id, tenant_position_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a tenant position
        api_response = api_instance.patch_tenant_position(tenant_id, tenant_position_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of PositionsApi->patch_tenant_position:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling PositionsApi->patch_tenant_position: %s\n" % e)
```
