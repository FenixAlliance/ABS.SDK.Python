...
# **patch_tenant_option**
> EmptyEnvelope patch_tenant_option(tenant_id, option_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a tenant option
        api_response = api_instance.patch_tenant_option(tenant_id, option_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of OptionsApi->patch_tenant_option:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling OptionsApi->patch_tenant_option: %s\n" % e)
```
