...
> EmptyEnvelope patch_shipping_region_async(tenant_id, region_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a shipping region
        api_response = api_instance.patch_shipping_region_async(tenant_id, region_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of ShippingRegionsApi->patch_shipping_region_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ShippingRegionsApi->patch_shipping_region_async: %s\n" % e)
```
