...
> EmptyEnvelope patch_shipping_class_async(tenant_id, class_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a shipping class
        api_response = api_instance.patch_shipping_class_async(tenant_id, class_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of ShippingClassesApi->patch_shipping_class_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ShippingClassesApi->patch_shipping_class_async: %s\n" % e)
```
