...
> EmptyEnvelope patch_course_update_async(tenant_id, update_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course update
        api_response = api_instance.patch_course_update_async(tenant_id, update_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseUpdatesApi->patch_course_update_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseUpdatesApi->patch_course_update_async: %s\n" % e)
```
