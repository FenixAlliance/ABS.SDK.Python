...
> EmptyEnvelope patch_course_cohort_async(tenant_id, cohort_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course cohort
        api_response = api_instance.patch_course_cohort_async(tenant_id, cohort_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseCohortsApi->patch_course_cohort_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseCohortsApi->patch_course_cohort_async: %s\n" % e)
```
