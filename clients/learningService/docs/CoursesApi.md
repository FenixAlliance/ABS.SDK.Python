...
# **patch_course_async**
> EmptyEnvelope patch_course_async(tenant_id, course_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course
        api_response = api_instance.patch_course_async(tenant_id, course_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CoursesApi->patch_course_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CoursesApi->patch_course_async: %s\n" % e)
```
