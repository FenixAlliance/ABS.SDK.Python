...
> EmptyEnvelope patch_course_wiki_async(tenant_id, wiki_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course wiki
        api_response = api_instance.patch_course_wiki_async(tenant_id, wiki_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseWikisApi->patch_course_wiki_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseWikisApi->patch_course_wiki_async: %s\n" % e)
```
