...
> EmptyEnvelope patch_course_assignment_type_async(tenant_id, assignment_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course assignment type
        api_response = api_instance.patch_course_assignment_type_async(tenant_id, assignment_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseAssignmentTypesApi->patch_course_assignment_type_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseAssignmentTypesApi->patch_course_assignment_type_async: %s\n" % e)
```
