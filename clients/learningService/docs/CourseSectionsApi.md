...
> EmptyEnvelope patch_course_section_async(tenant_id, section_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course section
        api_response = api_instance.patch_course_section_async(tenant_id, section_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseSectionsApi->patch_course_section_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseSectionsApi->patch_course_section_async: %s\n" % e)
```
