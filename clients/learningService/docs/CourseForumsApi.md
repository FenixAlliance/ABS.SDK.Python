...
> patch_course_forum_async(tenant_id, forum_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course forum
        api_instance.patch_course_forum_async(tenant_id, forum_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
    except Exception as e:
        print("Exception when calling CourseForumsApi->patch_course_forum_async: %s\n" % e)
```
