...
> EmptyEnvelope patch_course_assignment_async(tenant_id, assignment_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course assignment
        api_response = api_instance.patch_course_assignment_async(tenant_id, assignment_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseAssignmentsApi->patch_course_assignment_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseAssignmentsApi->patch_course_assignment_async: %s\n" % e)
```
