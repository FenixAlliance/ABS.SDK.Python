...
> patch_course_handout_async(tenant_id, handout_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course handout
        api_instance.patch_course_handout_async(tenant_id, handout_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
    except Exception as e:
        print("Exception when calling CourseHandoutsApi->patch_course_handout_async: %s\n" % e)
```
