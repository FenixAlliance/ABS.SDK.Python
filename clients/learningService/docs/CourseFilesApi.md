...
> patch_course_file_async(tenant_id, file_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course file
        api_instance.patch_course_file_async(tenant_id, file_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
    except Exception as e:
        print("Exception when calling CourseFilesApi->patch_course_file_async: %s\n" % e)
```
