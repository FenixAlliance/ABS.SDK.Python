...
> EmptyEnvelope patch_course_category_async(tenant_id, category_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a course category
        api_response = api_instance.patch_course_category_async(tenant_id, category_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of CourseCategoriesApi->patch_course_category_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CourseCategoriesApi->patch_course_category_async: %s\n" % e)
```
