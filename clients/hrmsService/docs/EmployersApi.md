...
> EmptyEnvelope patch_employer_async(tenant_id, employer_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch an employer
        api_response = api_instance.patch_employer_async(tenant_id, employer_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of EmployersApi->patch_employer_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling EmployersApi->patch_employer_async: %s\n" % e)
```
