...
# **patch_shift_async**
> EmptyEnvelope patch_shift_async(tenant_id, shift_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a shift
        api_response = api_instance.patch_shift_async(tenant_id, shift_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of ShiftsApi->patch_shift_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ShiftsApi->patch_shift_async: %s\n" % e)
```
