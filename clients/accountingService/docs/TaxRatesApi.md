...
# **patch_tax_rate**
> EmptyEnvelope patch_tax_rate(tenant_id, id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a tax rate
        api_response = api_instance.patch_tax_rate(tenant_id, id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of TaxRatesApi->patch_tax_rate:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling TaxRatesApi->patch_tax_rate: %s\n" % e)
```
