...
> EmptyEnvelope patch_fiscal_period_async(tenant_id, fiscal_period_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a fiscal period
        api_response = api_instance.patch_fiscal_period_async(tenant_id, fiscal_period_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of FiscalPeriodsApi->patch_fiscal_period_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling FiscalPeriodsApi->patch_fiscal_period_async: %s\n" % e)
```
