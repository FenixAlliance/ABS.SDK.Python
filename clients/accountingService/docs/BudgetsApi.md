...
> Int32Envelope get_budgets_count_async(tenant_id, api_version=api_version, x_api_version=x_api_version)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get the count of budgets
        api_response = api_instance.get_budgets_count_async(tenant_id, api_version=api_version, x_api_version=x_api_version)
        print("The response of BudgetsApi->get_budgets_count_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling BudgetsApi->get_budgets_count_async: %s\n" % e)
```
