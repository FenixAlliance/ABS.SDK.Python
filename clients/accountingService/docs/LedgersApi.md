...
# **patch_ledger_async**
> EmptyEnvelope patch_ledger_async(tenant_id, ledger_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patches a ledger
        api_response = api_instance.patch_ledger_async(tenant_id, ledger_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of LedgersApi->patch_ledger_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling LedgersApi->patch_ledger_async: %s\n" % e)
```
