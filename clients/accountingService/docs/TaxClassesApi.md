...
# **patch_tax_class**
> EmptyEnvelope patch_tax_class(tenant_id, id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a tax class
        api_response = api_instance.patch_tax_class(tenant_id, id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of TaxClassesApi->patch_tax_class:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling TaxClassesApi->patch_tax_class: %s\n" % e)
```
