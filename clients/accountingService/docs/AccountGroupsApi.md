...
> EmptyEnvelope patch_account_group_async(tenant_id, account_group_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch an account group
        api_response = api_instance.patch_account_group_async(tenant_id, account_group_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of AccountGroupsApi->patch_account_group_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AccountGroupsApi->patch_account_group_async: %s\n" % e)
```
