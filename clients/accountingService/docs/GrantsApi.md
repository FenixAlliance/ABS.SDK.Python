...
# **patch_grant_async**
> EmptyEnvelope patch_grant_async(tenant_id, grant_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a grant
        api_response = api_instance.patch_grant_async(tenant_id, grant_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of GrantsApi->patch_grant_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling GrantsApi->patch_grant_async: %s\n" % e)
```
