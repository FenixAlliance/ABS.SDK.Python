...
# **patch_expense_type**
> EmptyEnvelope patch_expense_type(tenant_id, expense_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch an expense type
        api_response = api_instance.patch_expense_type(tenant_id, expense_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of ExpenseTypesApi->patch_expense_type:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ExpenseTypesApi->patch_expense_type: %s\n" % e)
```
