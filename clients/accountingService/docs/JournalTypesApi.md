...
> EmptyEnvelope patch_journal_type_async(tenant_id, journal_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a journal type
        api_response = api_instance.patch_journal_type_async(tenant_id, journal_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of JournalTypesApi->patch_journal_type_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling JournalTypesApi->patch_journal_type_async: %s\n" % e)
```
