...
> EmptyEnvelope patch_warehouse_async(tenant_id, warehouse_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a warehouse
        api_response = api_instance.patch_warehouse_async(tenant_id, warehouse_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of WarehousesApi->patch_warehouse_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling WarehousesApi->patch_warehouse_async: %s\n" % e)
```
