...
> EmptyEnvelope patch_support_request_attachment_async(tenant_id, support_request_attachment_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a support request attachment
        api_response = api_instance.patch_support_request_attachment_async(tenant_id, support_request_attachment_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of SupportRequestAttachmentsApi->patch_support_request_attachment_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SupportRequestAttachmentsApi->patch_support_request_attachment_async: %s\n" % e)
```
