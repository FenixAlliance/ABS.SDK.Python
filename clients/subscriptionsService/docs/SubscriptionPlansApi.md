...
> EmptyEnvelope patch_subscription_plan_async(tenant_id, plan_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a subscription plan
        api_response = api_instance.patch_subscription_plan_async(tenant_id, plan_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of SubscriptionPlansApi->patch_subscription_plan_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SubscriptionPlansApi->patch_subscription_plan_async: %s\n" % e)
```
