...
> Envelope patch_service_queue_async(tenant_id, service_queue_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a service queue
        api_response = api_instance.patch_service_queue_async(tenant_id, service_queue_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of ServiceQueuesApi->patch_service_queue_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ServiceQueuesApi->patch_service_queue_async: %s\n" % e)
```
