...
> Envelope patch_service_level_async(tenant_id, service_id, service_level_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a service level
        api_response = api_instance.patch_service_level_async(tenant_id, service_id, service_level_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of ServiceLevelsApi->patch_service_level_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ServiceLevelsApi->patch_service_level_async: %s\n" % e)
```
