...
> EmptyEnvelope patch_payment_method_async(tenant_id, payment_method_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a payment method
        api_response = api_instance.patch_payment_method_async(tenant_id, payment_method_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of PaymentMethodsApi->patch_payment_method_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling PaymentMethodsApi->patch_payment_method_async: %s\n" % e)
```
