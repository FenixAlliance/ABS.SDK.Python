...
# **patch_system_option**
> EmptyEnvelope patch_system_option(option_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Partially update a system option
        api_response = api_instance.patch_system_option(option_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of OptionsApi->patch_system_option:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling OptionsApi->patch_system_option: %s\n" % e)
```
