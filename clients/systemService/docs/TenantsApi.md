...
# **patch_tenant**
> EmptyEnvelope patch_tenant(tenant_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Partially update a specific tenant by ID.
        api_response = api_instance.patch_tenant(tenant_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of TenantsApi->patch_tenant:\n")
        pprint(api_response)
    except Exception as e:
