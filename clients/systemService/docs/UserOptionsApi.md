...
> EmptyEnvelope patch_system_user_option(user_id, option_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Partially update a user option (admin)
        api_response = api_instance.patch_system_user_option(user_id, option_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of UserOptionsApi->patch_system_user_option:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling UserOptionsApi->patch_system_user_option: %s\n" % e)
```
