...
# **patch_system_portal**
> EmptyEnvelope patch_system_portal(portal_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Partially update a system portal
        api_response = api_instance.patch_system_portal(portal_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of PortalsApi->patch_system_portal:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling PortalsApi->patch_system_portal: %s\n" % e)
```
