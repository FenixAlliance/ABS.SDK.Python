# coding: utf-8

"""
    QuotesService

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 2.0.0.0
    Contact: support@fenix-alliance.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.quote_dto_list_envelope import QuoteDtoListEnvelope

class TestQuoteDtoListEnvelope(unittest.TestCase):
    """QuoteDtoListEnvelope unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> QuoteDtoListEnvelope:
        """Test QuoteDtoListEnvelope
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `QuoteDtoListEnvelope`
        """
        model = QuoteDtoListEnvelope()
        if include_optional:
            return QuoteDtoListEnvelope(
                is_success = True,
                error_message = '',
                correlation_id = '',
                timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                activity_id = '',
                result = [
                    openapi_client.models.quote_dto.QuoteDto(
                        id = '', 
                        timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        closed = True, 
                        type = '', 
                        title = '', 
                        user_id = '', 
                        tenant_id = '', 
                        description = '', 
                        price_list_id = '', 
                        enrollment_id = '', 
                        individual_id = '', 
                        organization_id = '', 
                        receiver_tenant_id = '', 
                        first_name = '', 
                        last_name = '', 
                        company_name = '', 
                        billing_email = '', 
                        address_line1 = '', 
                        address_line2 = '', 
                        postal_code = '', 
                        country_id = '', 
                        state_id = '', 
                        city_id = '', 
                        customer_notes = '', 
                        tax_calculation_method = 'Included', 
                        cost_calculation_method = 'Automatic', 
                        forex_rate = 1.337, 
                        currency_id = '', 
                        total_detail = 1.337, 
                        total_detail_currency_id = '', 
                        total_profit = 1.337, 
                        total_profit_currency_id = '', 
                        total_discounts = 1.337, 
                        total_discounts_currency_id = '', 
                        total_surcharges = 1.337, 
                        total_surcharges_currency_id = '', 
                        total_tax_base = 1.337, 
                        total_tax_base_currency_id = '', 
                        total_taxes = 1.337, 
                        total_taxes_currency_id = '', 
                        total_shipping_cost = 1.337, 
                        total_shipping_cost_currency_id = '', 
                        total_shipping_tax = 1.337, 
                        total_shipping_tax_currency_id = '', 
                        total_withheld_tax = 1.337, 
                        total_withheld_tax_currency_id = '', 
                        total_global_discounts = 1.337, 
                        total_global_discounts_currency_id = '', 
                        total_global_surcharges = 1.337, 
                        total_global_surcharges_currency_id = '', 
                        total = 1.337, 
                        total_currency_id = '', 
                        total_detail_in_usd = 1.337, 
                        total_profit_in_usd = 1.337, 
                        total_discounts_in_usd = 1.337, 
                        total_surcharges_in_usd = 1.337, 
                        total_tax_base_in_usd = 1.337, 
                        total_taxes_in_usd = 1.337, 
                        total_withheld_taxes_in_usd = 1.337, 
                        total_shipping_cost_in_usd = 1.337, 
                        total_shipping_taxes_in_usd = 1.337, 
                        total_global_discounts_in_usd = 1.337, 
                        total_global_surcharges_in_usd = 1.337, 
                        total_in_usd = 1.337, 
                        cart_id = '', 
                        deal_unit_id = '', 
                        effective_to = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        effective_from = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        quote_status = 'Draft', 
                        freight_terms = 'FOB', 
                        custom_discounts_amount = 1.337, )
                    ]
            )
        else:
            return QuoteDtoListEnvelope(
        )
        """

    def testQuoteDtoListEnvelope(self):
        """Test QuoteDtoListEnvelope"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
