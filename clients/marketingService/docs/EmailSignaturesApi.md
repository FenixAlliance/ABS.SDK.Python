...
> EmptyEnvelope patch_email_signature_async(tenant_id, emailsignature_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch an email signature
        api_response = api_instance.patch_email_signature_async(tenant_id, emailsignature_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of EmailSignaturesApi->patch_email_signature_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling EmailSignaturesApi->patch_email_signature_async: %s\n" % e)
```
