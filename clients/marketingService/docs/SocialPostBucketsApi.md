...
> EmptyEnvelope patch_social_post_bucket_async(tenant_id, socialpostbucket_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch a social post bucket
        api_response = api_instance.patch_social_post_bucket_async(tenant_id, socialpostbucket_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
        print("The response of SocialPostBucketsApi->patch_social_post_bucket_async:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SocialPostBucketsApi->patch_social_post_bucket_async: %s\n" % e)
```
