...
> patch_item_review_async(tenant_id, item_review_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch an item review
        api_instance.patch_item_review_async(tenant_id, item_review_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
    except Exception as e:
        print("Exception when calling ItemReviewsApi->patch_item_review_async: %s\n" % e)
```
