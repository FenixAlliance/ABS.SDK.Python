...
> patch_item_type_async(tenant_id, item_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Patch an item type
        api_instance.patch_item_type_async(tenant_id, item_type_id, api_version=api_version, x_api_version=x_api_version, operation=operation)
    except Exception as e:
        print("Exception when calling ItemTypesApi->patch_item_type_async: %s\n" % e)
```
